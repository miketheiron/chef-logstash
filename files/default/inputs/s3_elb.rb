...
class LogStash::Inputs::S3Elb < LogStash::Inputs::Base
  config_name "s3_elb"
  milestone 1
  ...
  # initial since_date from which to process logs
  config :since_date, :validate => :string, :default => nil
  ...
  def register
    ...
    @logger.info("Registering S3 ELB input", :bucket => @bucket, :region_endpoint => @region_endpoint, :prefix => @prefix)
    ...
    @since_date = Time.parse(@since_date) unless @since_date.nil?
  ...
  def process_new(queue, since=nil)
    ...
    if since.nil?
      ...
      unless @since_date.nil?
        since = [since, @since_date].max
      end
    end

    # don't iterate over days if we don't have a starting time
    range = since == Time.new(0) ? [nil] : (since.to_date .. Time.now.to_date)

    range.each do |day|
      @logger.debug("S3 ELB input processing daily logs", :bucket => @bucket, :prefix => @prefix, :day => day.to_s)

      objects = list_new(since, day)
      objects.each do |k|
        @logger.debug("S3 ELB input processing log", :bucket => @bucket, :day => day.to_s, :key => k)
  ...
  private
  def list_new(since, day=nil)

    objects = {}

    prefix = day.nil? ? @prefix : "#{@prefix}#{day.year}/#{day.month.to_s.rjust(2,'0')}/#{day.day.to_s.rjust(2,'0')}/"
